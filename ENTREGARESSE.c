...
//RA: 2648938
/*INSTRUÇÕES:
O código tem algumas necessidades:
 - é necessário que não cadastre disciplinas com o código 0.
 - não use char para a definição da ação no menu
E algumas especificações:
 - o Vetor alunos é alocado inicialmente com 3 espaços, pois fiz o cadastro automático para auxiliar na verificações de funções, para que não fosse necessário cadastrar alunos para teste.
 - a mesma coisa para compromissos, só que foram cadastrados 7 compromissos
 - e a mesma coisa para disciplinas, e foram cadastrados 15, e ao realocar disciplinas com a função, a função dobra o tamanho de memória.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void cadastra_aluno (aluno **alunos, int *num_alunos, int *tamanho_alunos, disciplina disciplinas[]) {
    ...
    {
        *tamanho_alunos+=10; //dobra o tamanho
...
void cadastra_compromisso(compromisso **compromissos, int *num_compromissos, int *tamanho_compromissos, aluno alunos[], int num_alunos) {
    ...
    {
        *tamanho_compromissos+=5; // Dobra o tamanho
